...
3. Trading parameters: mode (backtest or live), heartbeat duration, target index, start and end of backtest.
...
There exist several portfolio performance reporting packages on CRAN; it is beyond the scope of this exercise to replicate the functionality of those packages. The `4_report.R` script has three objectives - 
...
As present portoflio rebalancing occurs daily, and trading periodicity is set by the heartbeat duration. This causes problems for strategies that imply a lot of churn, such as a random rebalancing strategy. Because the weights are recalculated daily by default, the churn on these portfolios is significant, which overwhelms other characteristics that researchers may be interested in. Modifying the `trade.R` script to accept a parameter that varies rebalancing time would add significant flexibility to the researchers toolkit. This parameter could also be an input to the algorithm, as it effectively translates to a holding period specification and is amenable to hyperparameter tuning.
...
The cross-validation results would also benefit from being packaged into a parametrised .Rmd file, similar to the individual results tearsheets. The results of the cross validation are primarily intended for human consumption and rendering these results into a more human-friendly format such as an html or PDF report would facilitate ease of interpretation.
