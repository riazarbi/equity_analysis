...
Data mining, in particular, deserves some elaboration. It is standard practice to divide a dataset into a training and test set, and to train models on the training set (@Peterson2017). Once a profitable strategy has been found, it can be tested on the test set to determine how well it performs out of sample. Unfortunately, it is extemely easy to programatically tune backtest parameters to obtain a strategy that performs well in-sample. @Bailey2014 show how easy it is to derive high sharpe ratios over a random series by hyperparameter tuning. They claim that, with 5 years of data, only 45 trials can be tried before the expected in-sample sharpe ratio is 1, whereas the expected out of sample sharpe ratio is 0. A fundamental issue is that the likelihood of uncovering a spuriously high sharpe ratio increases with the number of trials. Using a hold-out test set naively guards against overfitting, but the limited amount of data (there is only one history) inevitably results in knowledge of the test set characteristics in in-smaple training. In the presence of memory effects in-sample performance is *negatively* correlated with out of sample performance.
...
3. Select all those combinations that are of the size $S/2$. That is, use those combinations which contain half of the returns, divided into segments 
2. Order each combination set of partitions $C$ chronologically and join them to form a training set $C_train$. The remaining partitions in the matrix $M$ are joined to form the testing set $C_test$, for this particular combination. $C_train$ and $C_test$ are the same dimensions, by design.
3. For each $C$, performance statistics are computed for each trial on the training set $C_train$ and the testing set $C_test$. Rank the trials accordingly. At the conclusion of this step, for each combination, the tester has a set of performance statistics in sample and out of sample, and the corresponding in and out of sample trials rankings.
4. For each of these combinations $C$, determine the best in-sample strategy. Determine the relative rank of these best in sample strategies out of sample. Relative rank is defined as $out of sample rank/number of trials + 1)$. The possible range of relative rank is therefore $[0-1]$.
5. For each of these combinations, compute the logit, where logit is the natural log of the relative rank divided by 1-relative rank $ln relative rank / (1-relative rank). Rankings which persist across in sample and out of sample performance will have a high logit value.
6. Collect all the logits from all the combinations, and detemrine the relative frequency of logit occurrence across the $C$ combinations. That is, determine a density function $F$ where $$integralF = 1$$.

$F$ can be visualised as a histrogram of the logits along with a density function.

The first of the tools presented is the *Probability of Backtest Overfitting* (PBO), which is the probability that a strategy with optimal performance in sample performs below the median out of sample. Simply put, the PBO is the proportion of $F$ along the domain $-infinity, 0$. The more of the density function that is below 0, the more likely it is that out of sample performance for the best in sample performer will be less than the median. 

The second is the *Performance Degradation*, which determines the correlation between performance in sample and out of sample. This is useful for determining if memory effects are present. Is is simply the regression of out of sample performance against in sample performance, and is typically shown as a scatter plot with a regression line. A negative regression slope indicates the presence of memory effects.
...
All code is available at [Github - Equity Analysis](https://github.com/riazarbi/equity_analysis).
...
It is intended that a researcher will clone this repository to a local directory and then create an RStudio project in the project root directory. After querying data from an appropriate vendor and processing the data using `scripts/1_query_source.R` and `scripts/2_process_data.R`, a researcher will specify global parameters in the `scripts/parameters.R` file and place a batch of algorithms in the `trials/` directory. Running the `scripts/3_run_trials.R` script will backtest each algorithm and save the results in the `results/` directory. Running `scripts/4_report.R` will generate a report on the outcome of each backtest and running `scripts/5_cross_validate` will run the R package `pbo` against the overall results to test for the probability of backtest overfitting.
...
The source code referenced in this section is released under the GNU General Public License v3.0. A full copy of the license is avaialable in the code repository at [Github - Equity Analysis](https://github.com/riazarbi/equity_analysis).
