...
Where possible, we have made I/O optimizations. This includes converting `.csv` files to `.feather` files, which are much faster to read and write, and parallelizing select prices of code. Ticker-wise data pipelining is multi-threaded. The trading engine is single-threaded, but asynchronous. That is, several trading engines can simultaneously consume the `trials` directory contents and backtest in parallel without collision.
...
As the script works through each subdirectory it compiles a cross-sectional dataframe that contains all the trials' daily returns. That is, a dataframe with a row for every date, and a column for the daily returns for each trial result. This dataframe is saved to `results/portfolio_values.feather`, where it can be consumed by the `5_cross_validate.R` cross-validation script.

It must be emphasised that the `portfolio_stats.feather` table and the tearsheet are minimal examples, and they could be heavily augmented. We decided not to flesh out a complete statistical summary because there are other R packages that can do that (see @Portfolioanalytics as an example). Our objective here is to lay a foundation that can be quite easily extended to serve the needs of a researcher.
