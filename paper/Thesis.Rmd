...
![S&P 500 Open, High, Closing and Low prices as reported by Google and Yahoo!. All price fields have higher average reported values in the Google source. [@Trice2017]\label{Table1}](/home/riaz/projects/equity_analysis/paper/data/data-quality-a-boxplot-1.png)
...
1) Has *market impact* been taken into account? Return anomalies may be related to illiquidity. That is to say, the returns of a stock (or class of stocks) may appear to be abnormal, but there exists no liquidity in the market to make a trade at the modelled price [@Harvey2017]. A realistic model will adjust for this by assuming minimal impact for only a small fraction of daily volume traded, increment the price if the trade size exceeds this volume, and respect the bid/ask spread when deciding that a trade matches.

2) Have *transaction costs* been taken into account? Similar to market impact, transaction costs are associated with every real trade but often ignored in research. The transaction costs depend on the commission structure, which varies by broker and exchange. A class of stocks may appear to exhibit abnormal returns but all suffer from high transaction costs because they are, for instance, penny stocks that all require an exorbitant minimum commission for small lots [@Loonat2018].
...
[@LopezdePrado2013] present several mathematical tools to test for backtest overfitting. All of these tools rely on a method that [@LopezdePrado2013] named Combinatorially Symmetric Cross-Validation (CSCV). This method requires that the following steps be performed:
...
2) Order each combination set of partitions $C$ chronologically and join them to form a training set $C_i^{train}$. The remaining partitions in the matrix $M$ are joined to form the testing set $C_i^{test}$, for this particular combination. $C_i^{train}$ and $C_i^{train}$ are the same dimensions, by design.
...
\begin{align}
\mbox{Relative Rank} =\frac{\mbox{OOS Rank of IS Winner}}{(\mbox{Number of Trials} + 1)} 
\end{align}
The possible range of relative rank is therefore $[0-1]$.  
5) For each of these combinations, compute the logit $\lambda_i$, where 
\begin{align}
\lambda = ln \frac{\mbox{Relative Rank}}{(1-\mbox{Relative Rank})}
\end{align}
...
2) Specifying breakpoints: Use NYSE breakpoints. Breakpoints are the market capitalization size boundaries according to which stocks are assigned to size bins.
...
Unfortunately, the aforementioned guidelines are not directly applicable to backtesting for trading. Traders are primarily interested in real-world excess returns; the use of CRSP prices to compute returns, for instance, does not translate into real cash balances because CRSP are not available on a real-time basis.
...
All code is available at https://github.com/riazarbi/equity_analysis [@Arbi2019].
...
It is not necessary to use a Docker image to run this code. Provided the user has installed the necessary dependencies, one can simply docwnload the code from the repository and execute it. However, this may impact the reproducibility of one's work - different users may have different versions of R or dependency packages; file path strings may behave differently on different operating systems. Encapsulating the code in a Docker container ensures that one can share a reproducible host environment for the code to execute in.

The codebase has been written with broad compatibility to the `rocker/tidyverse` docker image. Because of this, the `Dockerfile` in our repository uses `rocker/tidyverse` as a base image and merely adds a few specific packages. Rocker is the most popular docker image download on docker hub. Any machine that has docker installed can build a Docker image from this `Dockerfile` which will run the entire codebase with no additional configuration required. 
...
The directory structure loosely conforms to the [@Marwick2018] definition of a valid R packages structure, and most closely resembles the author's example of an intermediate-level compendium. The root directory contains the following files and directories: 
...
1) Connect to the vendor via an API (in the case of Bloomberg, this is via the [@Armstrong2018] R package Rblpapi).
...
knitr::kable(head(sample_file[,1:3]), caption = "An example of tidy data.")
...
In contrast to datalogs, datasets are stored in a ‘tall and narrow’ data structure. The following table is ‘short and wide’, with one row per date, and one column per attribute (we omit columns due to limited space. There are also timestamp and source columns in this table): 
...
knitr::kable(head(sample_file[,1:3]), caption="Short and wide data structure.")
```
...
knitr::kable(head(sample_file[,1:5], 10), caption="Tall and narrow data structure")
```


We do this conversion for a number of reasons. Firstly, it allows us to filter for a particular data point (i.e, a date, attribute, and ticker cell) much more quickly by simply filtering each column. This enables efficient deduplication, compaction and versioning, which reduces the dataset size dramatically and speeds load, write and processing operations. Secondly, it allows us great flexibility in the attributes we collect. Collecting variable numbers of attributes in the querying step results in tables with varying columns. Joining multiple files to create one dataset on 'short and wide' data would require column matching on every single join operation, which is expensive and error-prone. In contrast, columns are stable when ‘tall and narrow’. No matter what the original attributes are, or how they change across queries, we know that we will always have the same columns, namely `date`, `timestamp`, `source`, `metric` and `value`. 
...
If enabled, the `load_slow_moving_data.R` script will use the R built-in `kmeans` function to to cluster the fundamental metrics according to the number of occurrences of each metric in the population. The rationale here is that core financial data is updated far less frequently than financial ratios with a market component, and that there will therefore be far less entries of fundamental metric measurements than market-related ones. The k-means algorithm assigns metrics into either a high-frequency cluster (typically updated daily or weekly) or low-frequnecy cluster (updated quarterly, biannually or annually). Once the clusters have been computed, the cluster of metrics with the lowest number of occurrences (i.e., the low-frequency metrics) have the dates of their occurrences lag-adjusted, while the other cluster(s) are left unmodified.
...
# Application and examples\label{Application}
...
3. Zip each backtest.
4. `docker run ... riazarbi/equity_analysis` a new backtest environment. This environemt already contains the code from the GitHub repository mentioned above.
...
In general, a backtest takes about 90 minutes per algortihm in the trials directory. That is, a 50-trial backtest will take up to 75 hours, if rebalanced daily and traded multiple times per day. These can be run in parallel, however, if one varies parameters to investigate outcomes in a range of trading conditions.
...
The following paragraphs walk through a typical data quality assessment. The data source in this example is Bloomberg, and the query method is via the `Rblpapi` package. 
...
Once RStudio has been loaded on the terminal and the `equity_analyis` R project has been loaded, the user needs to log in to the Bloomberg user interface using provided credentials. This should start the `bbcomm.exe` background process, which is what R uses to communicate with Bloomberg. One can verify that `bbcomm.exe` is running by inspecting the Task Manager process panel.
...
Our example queries the JSE All Share Index and Top 40 Indexes for a date range of 240 months, beginning on the 27th of January 2019. We query 15 metadata fields, 26 market data fields and 79 fundamental fields. The fields specified can be inspected in the code repository of this project. The script queries all data relating to those metrics for the period 1 January 1960 to 27 January 2019. 
...
The data quality `html` report can be obtained from the `results/` directory. It is accompanied by a sidecar `.Rmd` file, which can be edited and re-run for exploratory analysis. The report contains some datalog summary statistics which validate that all the information in the datalogs are being transferred to the at-rest datasets. It also contains a market data field completeness chart, which provides a visual representation of the proportion of each market metric that contain NA values across the dataset. We extract and discuss some of the figures from the report below.

```{r, fig.height = 4, fig.cap="Data health report excerpt: Market metric field completeness, full dataset"}
library(png)
library(grid)
img <- readPNG("/home/riaz/projects/equity_analysis/paper/data/market-metric-completeness.png")
grid.raster(img)
```

Figure 6.1 shows that some metrics are more complete than others. In particular, total return with gross dividends and market capitalization are largely complete from the year 2004 onwards. These metrics may be good candidates for portfolio performance scoring and the algorithm weighting, respectively. 

The report also shows the overall monthly market data `NA` percentages over time (Figure 6.2). `NA` values are problematic because they result in constituents being dropped from the dataset. Ideally a user will select the subset of metrics and the date range that minimizes this error while testing useful hypotheses.

```{r, fig.height = 4, fig.cap="Data health report excerpt: Monthly overall market NA over time"}
library(png)
library(grid)
img <- readPNG("/home/riaz/projects/equity_analysis/paper/data/market-overall-market-na-over-time.png")
grid.raster(img)
```
...
Fundamental data cannot be submitted to the same `NA` criterion as above becuse it is slow-moving, and hence has many `NA` values on a daily time scale. The backtester backfills fundamental data into daily values on the assumption that, if a stock has one fundamental data entry, it probably has a complete relevant set. Stocks with no fundamental data for a particular field have that field dropped, and, if they are required for the backtest, the ticker is excluded from the set. These exclusions are recorded in the console output at runtime. However since data is released on different schedules for different stocks, it is difficult to impute the best periodicity and aligment for assessing `NA` values. 

Another issue with fundamental data is lag adjustments. The amount of lag to be applied to fundamental data is specified in the `parameters.R` file. Some fundamental datasets contain a mix of daily and low-frequency data; the former does not need adjustment. To address this, the data loading script will auto-detect the number of clusters in the dataset, and, if there are two or more, apply the lag to only the least-frequently-updated set of metrics. This detection is done by performing K-Means analysis on the count of occurrences of each metric in the dataset for the `k` sequence 1-10. The average silhouete widths are plotted on a Figure 6.3 below. This figure indicates that our dataset does indeed have two clusters. The metrics contained in cluster 2 in the below cluster plot will have their dates automatically lag-adjusted by 120 days.

```{r, fig.height = 4, fig.cap="Data health report excerpt: K-means fundamental metric counts silhouette plot"}
library(png)
library(grid)
img <- readPNG("/home/riaz/projects/equity_analysis/paper/data/k-means-silhouette.png")
grid.raster(img)
```

Figure 6.4 below plots the cumulative number of occurrences of each metric, with the y-axis ordering the metrics in descending order of number of occurrences. Cluster 1 from the K-means clustering procedure are circled in red, and cluster 2 is circled in blue. Because of the large disparity in number of occurrences, it looks like cluster 2 contains metrics with occurrences equal to zero. This is not the case - it is just that daily metrics in this dataset occur so much more frequently than annual metrics that the x-axis is deceptive. The plot shows that the algorithm has indeed clustered by number of occurrences, and that cluster 1 includes all metrics with a number of occurrences greater than 1000, and cluster 2 including all occurrences less than 1000. 

Given the obviousness of these results, one may ask why K-means clustering is even necessary. It would be trivial for a human to view Figure 6.4 and sort the metrics into slow-moving or fast-moving metrics. The power of this method is that we have automated the process of lag detection and correction. Human lag detection and correction is time consuming and error prone, but fundamentally requires very little in the way of human judgement - it forms part of the manual drudgery of building high quality backtesting data. The lag correction procedure built here is completely automated, but can handle varying periodicities and consume an arbitrary number of metrics. It therefore represents a very general approach to automated lag detection.

```{r, fig.height = 4, fig.cap="Data health report excerpt: Fundamental metric clusters"}
library(png)
library(grid)
img <- readPNG("/home/riaz/projects/equity_analysis/paper/data/fundamental-metric-clusters.png")
grid.raster(img)
```

The data health report also show histograms of the `NA` proportions per market metric group per ticker (not shown here). The groups are overall market fields, algorithm-related fields, price-related fields, volume, and last price. This allows the user to discern the distribution of `NA` values across the constituents. One wants as many of the tickers as possible to have as low `NA` counts as possible, because this means less tickers will be dropped from the dataset at runtime.

Having reviewed the data health report on the full raw dataset, we can filter the dataset to exclude poorly represented metrics. The following market metric completeness chart shows a dataset that excludes all market metrics except price, volume and market cap metrics. We see that these cherry-picked metrics have a very low proportion of `NA` entries. This increases the likelihood of a backtest result that is a meaningful analogue of reality, because we are less likely to introduce sampling bias. Sampling bias would occur when we use a metric in our dataset that has a high proportion of `NA` entries for particular tickers. If the quality of the data for a particular metric is related to, say, quality of reporting from the underlying company, then the algorithm will silently drop lower-quality companies, resulting in a misleading backtest result.

```{r, fig.height = 4, fig.cap="Data health report on constrained dataset excerpt: Market metric field completeness, filtered dataset"}
library(png)
library(grid)
img <- readPNG("/home/riaz/projects/equity_analysis/paper/data/market-metric-completeness-filtered.png")
grid.raster(img)
```
...
```{r, fig.height = 4, fig.cap="Data health report on constrained dataset excerpt: Monthly overall market NA over time, filtered dataset"}
library(png)
library(grid)
img <- readPNG("/home/riaz/projects/equity_analysis/paper/data/market-overall-market-na-over-time_filtered.png")
grid.raster(img)
```
...
In order to ensure that data quality is not an issue, we run these backtests against synthetically-generated data, which can be replicated by running `1_simulated_to_datalog.R` prior to running the catchall `0_all.R` script. 
...
These backtests are not designed to be profitable. They are designed to validate that the following features of the backtesting engine are operating reliably:

1) Does the trading engine effectively trade towards a target weighting? For this to be the case, a portfolio with a target portfolio weighted identically to a market-weighted index and no transaction costs should perform almost identically to said index if it is reweighted as frequently as the underlying index is. We would expect portfolios with less frequent reweighting heartbeats to have a lower correlation with the index, since the algorthm would spend at least some of the time targeting a stale target basket weighting.
2) Do transaction costs impact portfolio in an expected manner? For this to be the case, portfolios that are identical with regards to trading rules, but which have varying transaction costs, should vary in performance strictly in order of their respective trading costs. There are two aspects to trading costs modeled in this engine - commissions as a percentage of trade, and minimum commissions. We test these separately.

|Trial ID| Variation                     | Expected Result                      | 
|-----------:|:--------------------------|:---------------------------|
|A	         |	4 years, no transaction costs, rebalance daily, trade daily. Weight all tickers by market cap.| Baseline. |
|B	         |	Same as A but rebalance monthly. | Expect lower correlation with index than A.|
|C	         |	Same as A but rebalance quarterly. |  Expect lower correlation with index than B. |
|D	         |	Same as A but 0.5% transaction costs. |  Expect lower performance than A. |
|E	         |	Same as A but 5% transaction costs. | Expect lower performance than D. |
|F	         |	Same as E but portfolio 10000 and minimum commission of 10 units. | Expect transaction costs to consume portolio capital until the portfolio value is zero. |
Table: Description of the characteristics of each trial conducted in the engine validation exercise.\label{Table3}

Each of the above portfolios employ a passive market weighting algorithm, similar to that employed by an index-tracking passive ETF. At each rebalancing heartbeat, the algorithm calculates the optimal weights to allocate to each security in order to mimic the weightings in the underlying index. At each trading heartbeat, the engine compares the portfolio actual portfolio weights weights with the target weights and generates a series of required trades to equalize these two lists. This series of trades is submitted to a trading engine, which matches trades if there is sufficient liquidity at the limit price. The parameters, algorithm code and results of the above backtests are available at https://github.com/riazarbi/equity_analysis_trials. 

The correlation table below (Table 6.2) confirms the expected result from the table above. Trial A has the greatest correlation with the market return, followed by trial B, then trial C. Firstly, we would expect trial A to closely track the market return, since the constituent list is recalculated daily, and there are no transaction costs. Any deviation should be attributed to liquidity constraints slowing the rotating of positions. Secondly, increasing the time window between rebalancing heartbeats results in greater deviation from the performance of the index. This is what one would expect, since constituents are constantly being rotated out of the index, and a slower rebalancing portfolio will track a stale constituent list for longer. The results indicate that the rebalancing parameter is working as intended.

```{r}
library(feather)
correl <- readRDS("/home/riaz/projects/equity_analysis/paper/data/correlation.Rds") 
knitr::kable(correl, caption = "Identical Portfolios, Varying Rebalancing Periodicity, Return Correlations")
```

With regard to the transaction costs test (see Figure 6.7 below), trial A strictly dominates trial D, which strictly dominates trial E. As expected, trial D is much closer to trial A than trial E. This indicated that increasing transaction costs decreases the return. This is in accordance with a properly functioning trading engine, since higher transaction costs equate to lower net realized gain on each trade.

```{r, fig.height = 4, fig.cap="Identical portolios, varying transaction costs"}
library(png)
library(grid)
img <- readPNG("/home/riaz/projects/equity_analysis/paper/data/transaction_costs_validation.png")
grid.raster(img)
```

The low-value, high-minimum-commission portfolio (portfolio F, Figure 6.7 below) also behaves as expected, descending into bankruptcy. This can be seen by examining the Indexed Value axis, which descends to a negative number almost immediately. We expected this strategy to descend into bankruptcy because the transaction cost was very high compared to the portfolio capital value; each trade would consume at least 0.1% of the capital of the portfolio. The [tearsheet for this portfolio](https://github.com/riazarbi/equity_analysis_trials/blob/master/backtests/13/results/simulated__RISK_FREE_RANDOMS__all_select_market_weight/) indicates that it completed 442 trades.

Even though the portfolio entered negative net worth in the first month of trading, it did manage to trade sporadically until the end of the backtest window, whenever a stock it happened to own could be swapped for a desirable stock in the same heartbeat. In reality, a bankrupt portoflio would probably be liquidated, but no liquidation functionality has been built into this backtester at this time. This result suggests that the code would benefit from better bankruptcy handling. 

```{r, fig.height = 4, fig.cap="Identical portolios, smaller portfolio size and high minimum commission"}
library(png)
library(grid)
img <- readPNG("/home/riaz/projects/equity_analysis/paper/data/low_value_high_cost_returns.png")
grid.raster(img)
```
...
As detailed in \ref{Backtest Overfitting}, running many trials on the same dataset seriously increases the risk of backtest overfitting. This is especially risky for a process that is amenable to parameter tuning (varying rebalancing periods, portfolio sizes and weighting rules and the like). For this reason, we include a backtest overfitting report in the `results/` directory. In an earnest backtesting exercise one would want to avoid backtest overfitting, since backtests which are overfit do not have much predictive power for the future. The portfolios generated in this exercise have been explicitly designed to result in a failure to pass a PBO test. That is, we expect to confirm that backtest overfitting has occurred. 
...
In order to demonstrate this functionality, we simulate a set of portfolio returns and portfolio values that would have been derived from truly random returns. We do this by creating an R `data frame` with 50 columns and 2000 rows; each of these cells has a random sample taken from the standard normal distribution. Each column represents a portfolio and each row a date. This data frame therefore represents a series of random daily returns for 50 portfolios. We adjust the standard deviation of the random returns to a reasonable daily rate ($sd = \frac{1}{365^2}$) to make the distribution comparable to a real-world daily compounding rate. We assign a dates column to the `data frame` and then save it do disk, in the location that the cross validation script looks for portfolio array returns. This data frame can be recreated by executing the code found [here.](https://github.com/riazarbi/equity_analysis_trials/blob/master/backtests/PBO/workings.R)

Next we create a portfolio value data frame from the returns data frame by converting each column into a rolling compounding column:

\begin{align}
\mbox{price}_t = (1+\mbox{return}_t)*(\mbox{price}_{t-1})
\end{align}

We save this data frame to disk in the expected location as well. Next, we run `6_cross_validate.R` and inspect the html report, which can be seen [here.](https://github.com/riazarbi/equity_analysis_trials/tree/master/backtests/PBO) The indexed returns plot from the report is reproduced below. It shows that as a group, these portfolios indexed values range between ~140% and ~70% of their initial values over the time period. However, there is no discernible performance trend. This is expected, as the daily returns were not the result of any algorithm but rather randomly sampled from the normal distribution.

```{r, fig.height = 4, fig.cap="Excerpt from cross-validation report: Portfolio returns time series, 50 portfolios with random daily returns"}
library(png)
library(grid)
img <- readPNG("/home/riaz/projects/equity_analysis/paper/data/random_returns.png")
grid.raster(img)
```

Once the cross validation report has been generated we can inspect it to assess the probability of backtest overfitting. The excess return metric we use is the same as the metric used in [@LopezdePrado2013] - the Sharpe Ratio. This ratio is defined as 

\begin{align}
\mbox{Sharpe} = \frac{\mathbb{E}\ (\mbox{Excess Return})}{\mathrm{Std}(\mbox{Excess Return})}
\end{align}

where the Excess Return is the portfolio return over and above the risk free return.

The results of the PBO exercise are summarized in Table 6.3 below. One should interpret the table as follows. PBO refers the the probability of backtest overfitting, as detailed in Section \ref{Backtest Overfitting}. A PBO value close to 1 indicates that the cross validation exercise has detected that overfitting has probably taken place. Slope refers to the slope of the regression of IS performance to OOS performance. A negative slope indicates that positive IS performance is related to negative OOS performance. The Adjusted R-squared tells us how well the slope describes that relationship between IS and OOS performance - a high value means we can ascribe a high degree of confidence to the slope coefficient. The Probability of Loss captures the probability of a loss occurring if one were to employ the highest-ranked IS portfolio's strategy on OOS data. The results in Table 6.3 tell us that the probability of backtest overfitting is very high, that IS returns are weakly negatively predictive of OOS returns, and that the probability of loss is high. This is in accordance with our expectations, since the returns that we generated for all portfolios were random. Because they were random, the best-performing portfolio for any period will not necessarily perform better than the median portfolio OOS. Essentially the IS returns contain no information about future performance, except that high IS returns are moderately weakly predictive of poor OOS performance, possibly due to reversion to the mean in the returns matrix.

```{r}
results <- c(1,-0.00008,0.98,1)
names(results) <- c("PBO", "Slope", "Adjusted R-squared", "Probability of Loss")
colnames_summary <- c("Result", "Possible Range", "Desirable Value")
summary_Range <- c("0->1", "-inf->inf", "0->1", "0->1")
summary_Desirable  <- c("0", "NA", "1", "0")
knitr::kable(cbind(results, summary_Range, summary_Desirable), col.names=colnames_summary, caption="PBO Results")
```


The histogram of logits confirm that the best in-sample trial performed below the median out of sample trial. 

```{r, fig.height = 4, fig.cap="Excerpt from cross-validation report: Histogram of logits, random weighting algorithms"}
library(png)
library(grid)
img <- readPNG("/home/riaz/projects/equity_analysis/paper/data/histogram_of_logits.png")
grid.raster(img)
```
...
```{r, fig.height = 4, fig.cap="Excerpt from cross-validation report: Out of sample performance degradation, random weighting algorithms"}
library(png)
library(grid)
img <- readPNG("/home/riaz/projects/equity_analysis/paper/data/oos_degradation.png")
grid.raster(img)
```

Finally, the pairs plot shows little relationship between in-sample and out-of-sample performance.

```{r, fig.height = 4, fig.cap="Excerpt from cross-validation report: Pairs plot, random weighting algorithms"}
library(png)
library(grid)
img <- readPNG("/home/riaz/projects/equity_analysis/paper/data/pairs_plot.png")
grid.raster(img)
```
...
In line with our goal of enabling high-quality, reproducible research, it would be desirable to expand the data quality reporting to compare multiple data sources and make a 'est guess' synthetic dataset out of multiple fragmented sources.
...
It is hoped that researchers will treat this project as a reproducible project in and of itself. That is, they will inspect the code, ensure that it is working as intended (or suggest fixes) and thereby verify it as a legitimate base for engaging in the real meat of backtesting - discovering anomalies.
