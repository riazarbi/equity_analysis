...
The first step in constructing a backtest is data collection. Once the data has been collected, it needs to be assesed for quality. Typical questions to be asked include - 

- How likely is a researcher to obtain an accurate or representative result, given the quality of the data? 
- If an algorithm presumes that all data is available for each metric and for each ticker, what it the impact of a dataset that is missing entire ticker datasets on the accuracy of the result? 
...
This chart shows that some metrics are more complete than others. In particular, 
total return with gross dividends and market capitalization are largely complete from the year 2004 onwards. These metrics may be good candidates for portfolio performance scoring and the algorithm weighting, respectively. 
...
|13	         |	Same as 12 but portfolio 10000 and minimum commission of 10. Expect transaction costs to eat portolio. |
...
The results of the PBO execrcise are summarised below.

```{r}
results <- c(1,-0.00008,0.98,1)
names(results) <- c("PBO", "Slope", "Adjusted R-squared", "Probability of Loss")
colnames_summary <- "Value"
summary_Range <- c("0->1", "-inf->inf", "0->1", "0->1")
summary_Desirable  <- c("1", "NA", "1", "0")
knitr::kable(cbind(results, summary_Range, summary_Desirable), col.nmaes=colnames_summary, caption="PBO Results")
```
...
In line with our goal of enabling high-quality, reproducible research, it would be desirable to expand the data quality reporting to compare multiple data sources and make a 'est guess' synthetic dataset out of muliple fragmented sources.
