...
The first step in constructing a backtest is data collection. Once the data has been collected, it needs to be assesed for quality - how likely is a researcher to obtain an accurate or representative result, given the quality of the data? If an algorithm presumes that all data is available for each metric and for each ticker, what it the impact of a dataset that is missing entire ticker datasets on the accruacy of the result? What about data that contains all tickers, but is missing certain metrics? These sorts of issues are case-specific, so we provide an automated report that provides the researcher with measures of dataset health. The expectation is that the researcher will reflect on these measures and decide on an appropriate course of action. Appropriate courses of action could be -
...
The following paragraphs walk through a typical data quality assessment. They assume that the user has an already-initialized environment, as detailed in \ref{Application}. The data source in this example is Bloomberg, and the query method is via the `Rblpapi` package. 
...
Our example queries the JSE All Share Iddex and Top 40 Indexes for a date range of 240 months, beginning on the 27th of January 2019. We query 15 metadata fields, 26 market data fields and 79 fundamental fields. The fields specified can be inspected in the code repository of this project. The script queries all data relating to those metrics for the period 1 January 1960 to 27 January 2019. We ran our queries on the 27th January 2019.

Once these query parmeters have been set, we enter `source("scripts/1_bloomberg_to_datalog.R")` into the R console and press enter. The script takes approximately 30 minutes to run and saves over 14000 files to the datalog directory. Once complete, we transfer the removable media to a permanent workstation and transfer across the results.
...
The data quality `html` report can be obtained frmom the `results/` directory. It is accompanied by a sidecar `.Rmd` file, which can be edited and re-run for exploratory analysis. The report contains some datalog summary statistics which validate that all the information in the datalogs are being transferred to the at-rest datasets. It also contains a market data field completeness chart, which provides a visual representation of the proportion of each market metric that contain NA values across the dataset. 

```{r, fig.height = 4, fig.cap="Market Metric Field Completeness, Full Dataset"}
library(png)
library(grid)
...
This chart shows that some metrics are more complete than others. 
In particular, `TOT_RETURN_INDEX_GROSS_DVDS` and `CUR_MKT_CAP` are largely complete from the year 2004 onwards. These metrics may be good candidates for portfolio performance scoring and the algorithm weighting, respectively. 
...
```{r, fig.height = 4, fig.cap="Monthly Overall Market NA Over Time"}
library(png)
library(grid)
...
Another issue with fundamental data is lag adjustments. The amount of lag to be applied to fundamental data is specified in the `parameters.R` file. Some fundamental datasets contain a mix of daily and low-frequency data; the former does not need adjustment. To address this, the data loading script will auto-detect the number of clusters in the dataset, and, if there are two or mre, apply the lag to only the least-frequently-updated set of metrics. Our dataset does indeed have two lcusters, as shown by the k-means silhouette below. The metrics contained in cluster 2 in the below cluster plot have their dates lag-adjusted by 120 days.

```{r, fig.height = 4, fig.cap="K-means Fundamental Metric Counts Silhouette Plot"}
library(png)
library(grid)
...
```{r, fig.height = 4, fig.cap="Market Metric Field Completeness, Filtered Dataset"}
library(png)
library(grid)
...
The corresponding monthly `NA` timeseries plot is shown below. The best-quality period appears to be after 2010.
...
In order to validate that the backtester is functioning correctly, we run seven backtests on identical data, with an identical algorithm. 
...
|12	         |	Same as 8 but 5% transaction costs. Expect even lower performance than 8. |
|13	         |	Same as 12 but portfolio 10000 and minimum commisison of 10. Expect transaction costs to eat portolio. |
Table: Details of Each backtest Instance for Engine Validation Exercise\label{Table1}

The results of the above backtests are available at https://github.com/riazarbi/equity_analysis_trials.
...
With regard to the transaction costs test, trial 8 strictly dominates trial 11, with strictly dominates trial 12. As expected, trial 11 is much closer to trial 8 thna trial 12.
...
As detailed in \ref{Backtest Overfitting}, running many trials on the same dataset seriously increases the risk of backtest ovefitting. This is especially risky for a process that is amenable to parameter tuning (varying reabalancing periods, portfolio sizes and weighting rules and the like). For this reason, we include a backtest overfitting report in the `results/` directory. 

In this section we run a series of trials that randomly select stocks every rebalancing period and then weight them randomly. When the next rebalancing period occurs, a new portfolio set is generated, leading to a new set of target weights. This results in a lost of trading activity. Because these strategies have absolutely no underlying rule, we expect that there should be no correlation between in sample and out of sample performance. One caveat is that randomness can mask underlying systematic processes. In our initial tests we discovered that randomly weighting a fixed number of stocks resulted in correlated portfolio returns. This is because, in a situation where any stock is as likely to go up as go down, and we are weighting them randomly, the up movements tend to cancel the down movements in aggregate, and the portfolios behave much like an equally weighted portolio. For this reason our algorithms also randomly select the number of stocks to hold each period.

We divide our backtests across six environments. In each environment we run 50 identical trials. Because the backtests run in a detemerministic fashion, the random seed that we set in the `parameters.R` file ensures that these trials will run in lockstep. This property is useful for varying control parameters to precisely measure how they affect outcomes. in this case, however, we want as much diversity in behaviur as possible. 

Within each backtest enviroment, each trial should have no correlation with any other one. In order to introduce a diversity of returns across the backtest environments, we vary the rebalancing parameter. Because the weights are randomly reinitialized each period, varying the rebalancing periodicity will effectively break the lockstep behaviour of the environments.

|Trial ID| Variation                                            | Trials Completed|
|-----------:|:-------------------------------------------------|:----------------|
|2	         |	4 years, no transaction costs, rebalance daily, trade daily, random weight. |
|3	         |	Same as 2 but rebalance weekly.|
|4	         |	Same as 2 but rebalance quarterly. |
|5	         |	Same as 2 but rebalance yearly. ||
|6	         |	Same as 2 but never rebalance. |50|
Table: Details of Each Backtest Instance for PBO Demonstration\label{Table1}

Event-based backtesting is computationally expensive; after 24 hours we terminated the backtester trading process and proceeded to report generation.

The process for running these backtests in exactly the last section, with the exception that we interrrupt the console prematurely and then manually run `5_report.R` and `6_cross_validate.R`. Because the `cross_validate.R` script depends solely on the portfolio returns array, we can extract the portfolio array from each backtest environmment, join them, and re-run the `6_cross_validate.R` script on the joined array to obtain an overall cross validation report. The array of returns is plotted below. The best-performing portfolio appear to perfom substantially better than the risk free rate, and nively looks like a good candidate for selection.

```{r, fig.height = 4, fig.cap="Portfolio Returns Time Series, Random Weighting Algorithms"}
library(png)
library(grid)
...
Once the cross validation report has been generated we can inspect it so assess the probability of backtest overfitting. The excess return metric we use is the same as the metric used in @LopezdePrado2013 - the Sharpe Ratio. This ratio is defined as 

$$\Sharpe = \frac{ave\ (Excess\ Return)}{sd(Excess\ Return)}$$
Where the $Excess Return$ is the portfolio return over and above the risk free return.

```{r}
summary$results <- c(1,1,1,1)
...
knitr::kable(summary, caption="PBO Results")
```


The histogram of logits confirm that the best in-sample trial performed below the median out of sample trial. 

```{r, fig.height = 4, fig.cap="Histogram of Logits, Random Weighting Algorithms"}
library(png)
library(grid)
...
```{r, fig.height = 4, fig.cap="Out of Sample Performance Degradation, Random Weighting Algorithms"}
library(png)
library(grid)
...
Finally, tha pairs plot shows little relationship between in sample and out of sample performance.

```{r, fig.height = 4, fig.cap="Pairs Pot, Random Weighting Algorithms"}
library(png)
library(grid)
